# Libft - 42 | [FRENCH VERSION] _english below_

## 📚 Description

Libft est le premier projet de l'école 42.
L'objectif est de recréer un ensemble de fonctions de la bibliothèque standard C, ainsi que quelques fonctions supplémentaires utiles pour les futurs projets.

---

## 🛠️ Fonctions implémentées

### Partie 1 - Fonctions de la libc
- `ft_isalpha`  
- `ft_isdigit`  
- `ft_isalnum`  
- `ft_isascii`  
- `ft_isprint`  
- `ft_strlen`  
- `ft_memset`  
- `ft_bzero`  
- `ft_memcpy`  
- `ft_memmove`  
- `ft_strlcpy`  
- `ft_strlcat`  
- `ft_toupper`  
- `ft_tolower`  
- `ft_strchr`  
- `ft_strrchr`  
- `ft_strncmp`  
- `ft_memchr`  
- `ft_memcmp`  
- `ft_strnstr`  
- `ft_atoi`  
- `ft_calloc`  
- `ft_strdup`  

### Partie 2 - Fonctions supplémentaires
- `ft_substr`  
- `ft_strjoin`  
- `ft_strtrim`  
- `ft_split`  
- `ft_itoa`  
- `ft_strmapi`  
- `ft_striteri`  
- `ft_putchar_fd`  
- `ft_putstr_fd`  
- `ft_putendl_fd`  
- `ft_putnbr_fd`  

### Partie bonus (gestion des listes chaînées)
- `ft_lstnew`  
- `ft_lstadd_front`  
- `ft_lstsize`  
- `ft_lstlast`  
- `ft_lstadd_back`  
- `ft_lstdelone`  
- `ft_lstclear`  
- `ft_lstiter`  
- `ft_lstmap`  

---

## ⚙️ Compilation

| Commande      | Description                                   |
| ------------- | --------------------------------------------- |
| `make`        | Compile la bibliothèque et génère `libft.a`   |
| `make clean`  | Supprime les fichiers objets (`.o`)           |
| `make fclean` | Supprime les fichiers objets **et** `libft.a` |
| `make bonus` | Compile les exercices bonus                   |
| `make re`     | Supprime les fichiers ojets, la lib, puis recompile tout |

---

## 🖼️ Note du projet

<p align="center">
  <img width="199" height="169" alt="image" src="https://github.com/user-attachments/assets/4f3e91e7-6752-4e6a-b904-164c8a95f036" />
</p>

---

## 🫐 Auteur

* Océane (ocviller)
* Piscine de Mars 2025
* Projet réalisé à 42 Paris, 2025

-------------------------------------------------------------------------------------------------------------------------------------------

# Libft - 42 | [ENGLISH VERSION]

## 📚 Description

Libft is the first project at 42 School.
Its goal is to recreate a set of functions from the C standard library, along with several additional utility functions that will be useful for future projects.

---

## 🛠️ Implemented Functions

### Part 1 - Libc Functions
- `ft_isalpha`
- `ft_isdigit`
- `ft_isalnum`
- `ft_isascii`
- `ft_isprint`
- `ft_strlen`
- `ft_memset`
- `ft_bzero`
- `ft_memcpy`
- `ft_memmove`
- `ft_strlcpy`
- `ft_strlcat`
- `ft_toupper`
- `ft_tolower`
- `ft_strchr`
- `ft_strrchr`
- `ft_strncmp`
- `ft_memchr`
- `ft_memcmp`
- `ft_strnstr`
- `ft_atoi`
- `ft_calloc`
- `ft_strdup`

### Part 2 - Additional Functions
- `ft_substr`
- `ft_strjoin`
- `ft_strtrim`
- `ft_split`
- `ft_itoa`
- `ft_strmapi`
- `ft_striteri`
- `ft_putchar_fd`
- `ft_putstr_fd`
- `ft_putendl_fd`
- `ft_putnbr_fd`

### Bonus Part (Linked List Functions)
- `ft_lstnew`
- `ft_lstadd_front`
- `ft_lstsize`
- `ft_lstlast`
- `ft_lstadd_back`
- `ft_lstdelone`
- `ft_lstclear`
- `ft_lstiter`
- `ft_lstmap`

---

## ⚙️ Compilation

| Command | Description |
| ------- | ----------- |
| `make` | Compiles the library and generates `libft.a` |
| `make clean` | Removes object files (`.o`) |
| `make fclean` | Removes object files and `libft.a` |
| `make bonus` | Compiles the bonus functions |
| `make re` | Removes object files and the library, then recompiles everything |

---

## 🖼️ Project Grade

<p align="center">
  <img width="199" height="169" alt="Project grade: 125/100" src="https://github.com/user-attachments/assets/4f3e91e7-6752-4e6a-b904-164c8a95f036" />
</p>

---

## 🫐 Author

- Océane (`ocviller`)
- March 2025 Piscine
- Completed at 42 Paris in 2025
